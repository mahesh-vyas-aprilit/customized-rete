...
export type Props<Schemes extends BaseSchemes> = {
  ...
  ) => boolean | { color?: string; marker?: string; offset?: number };
};
 ...
 * @listens rendered
 */
export class ConnectionPathPlugin<Schemes extends BaseSchemes, K> extends Scope<
  never,
  [RenderProduces<Schemes>, ...Area2DInherited<Schemes, K>]
> {
  /**
   * @constructor
  ...
  override setParent(
    scope: Scope<RenderProduces<Schemes>, Area2DInherited<Schemes, K>>
  ): void {
    ...
    scope.addPipe((context) => {
      ...
      if (context.type === 'connectionpath') {
        ...
        const arrowOffset =
          typeof arrow === 'object' && arrow.offset !== undefined
            ? arrow.offset
            : -15;
        ...
        p.setAttribute(
          ...
          path ||
            classicConnectionPath(
              transformedPoints as [Position, Position],
              0.3
            )
        );
      ...
      if (context.type === 'rendered' && context.data.type === 'connection') {
        ...
        svgEl.setAttribute(
          'style',
          'position: absolute; overflow: visible !important; pointer-events: none'
        );
  ...
  private getArrowData(c: Schemes['Connection']) {
    ...
    if (!data) return null;
    const { color = 'steelblue', marker = 'M-5,-10 L-5,10 L20,0 z' } =
      data === true ? {} : data;
