 ...
 * @emits render
 */
export class DockPlugin<Schemes extends BaseSchemes> extends Scope<
  never,
  Area2DInherited<Schemes, never>
> {
  nodes = new Map<
    () => Schemes['Node'],
    { preset: Preset; element: HTMLElement }
  >();
  ...
  override setParent(scope: Scope<Area2D<Schemes>, [Root<Schemes>]>): void {
  ...
  add(
    ...
    for (const preset of this.presets) {
      const element = preset.createItem(name, icon, index);
      // const element = element1?.children.item(0) as HTMLElement;
      // console.log(element);
